...
static uint8_t DS18B20_write(void); // in bytes
...
static uint8_t DS18B20_reset(void)
{
    ...
    LL_USART_SetBaudRate(huart2.Instance, HAL_RCC_GetPCLK2Freq(), 9600);

    HAL_UART_Transmit(&huart2, &reset, 1, 1);
    HAL_UART_Receive(&huart2, &device, 1, 1);

    LL_USART_SetBaudRate(huart2.Instance, HAL_RCC_GetPCLK2Freq(), 115200);
...
static uint8_t DS18B20_readBit(void)
{
    ...
    HAL_UART_Transmit(&huart2, &readBit, 1, 1);
    HAL_UART_Receive(&huart2, &bit, 1, 1);
...
{
    DS18B20_reset();
    DS18B20_WriteByte(0xCC);  // Skip ROM [CCh] since only 1 sensor
    DS18B20_WriteByte(0x44);  // Convert T [44h] to start temp conversion
}
...
float DS18B20_readTemperature(void)
{
    ...
    DS18B20_reset();
    DS18B20_WriteByte(0xCC);  // Skip ROM [CCh] since only 1 sensor
    DS18B20_WriteByte(0xBE);  // Read Scratchpad [BEh] to get temp data
